	...
	Describe("space quotas", func() {
		It("can create, read, update, assign to a space, remove a space and delete a space quota", func() {
			AsUser(context.AdminUserContext(), 120*time.Second, func() {
				...
				space := Cf("space", spaceName).Wait(assertionTimeout)
				Expect(space).To(Exit(0))
				output = space.Out.Contents()
				Expect(output).To(ContainSubstring(fmt.Sprintf("Space Quota:")))
				Expect(output).To(ContainSubstring(fmt.Sprintf("foo")))
