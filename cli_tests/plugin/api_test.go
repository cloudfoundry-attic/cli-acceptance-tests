...
import (
	...
	cliTestHelpers "github.com/cloudfoundry/cli-acceptance-tests/helpers"
...
"github.com/cloudfoundry/cli-acceptance-tests/Godeps/_workspace/src/github.com/cloudfoundry-incubator/cf-test-helpers/generator"
)
...
var _ = BeforeSuite(func() {
	...
	Expect(runtime.GOARCH).To(Equal("amd64"), "Plugin suite only runs under 64bit OS, please skip the plugin suite in 32bit OS (use flag -skipPackage='tests/plugin')")
...
var _ = AfterSuite(func() {
	...
	uninstall := Cf("uninstall-plugin", "CliTestPlugin").Wait(5 * time.Second)
	...
	Describe("GetApp() and GetApps()", func() {
		It("gets app details and app list", func() {
			AsUser(context.RegularUserContext(), 250*time.Second, func() {
				...
				app1 := Cf("push", appName1, "-p", cliTestHelpers.NewAssets().ServiceBroker).Wait(appTimeout)
				Expect(app1).To(Exit(0))

				appName2 := generator.RandomName()
				app2 := Cf("push", appName2, "-p", cliTestHelpers.NewAssets().ServiceBroker).Wait(appTimeout)
